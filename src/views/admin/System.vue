<template>
  <div class="container">
    ...
    <div class="flex-container">
      ...
      <div>
        <p>System Email:
          <b>{{systemSettings.system_email }}</b>
        </p>
        <p>Email Password:
          <b>{{systemSettings.email_password }}</b>
        </p>
        <p>SMTP Host:
          <b>{{systemSettings.smtp_host }}</b>
        </p>
        <p>SMTP PORT:
          <b>{{systemSettings.smtp_port }}</b>
        </p>
        <p>TLS:
          <input v-if="systemSettings.email_tls" type="checkbox" checked>
          <input type="checkbox" v-else>
        </p>
        <hr>
        <h2>Recorder Einstellungen:</h2>
        <p>framesize:
          <b>{{systemSettings.framesize }}</b>
        </p>
        <p>Driver Path:
          <b>{{systemSettings.driver_path }}</b>
        </p>
        <p>Profile Path:
          <b>{{systemSettings.profile_path }}</b>
        </p>
